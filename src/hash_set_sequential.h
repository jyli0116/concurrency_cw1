...
class HashSetSequential : public HashSetBase<T> {
  public:
    explicit HashSetSequential(size_t initial_capacity): initial_capacity_(initial_capacity), set_size_(0) {
    ...
    bool Add(T elem) final {
      std::vector<T>& bucket_ = table_.at(std::hash<T>()(elem) % initial_capacity_);
  ...
  private: 
    ...
    bool Policy() {
      return set_size_ / initial_capacity_ > 4;
    }
    ...
    // used to represent the Hashset: a vector of vectors of type T: each vector representing the bucket
